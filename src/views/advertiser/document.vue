...
export default {
  ...
  methods: {
    // upload advertiser document method
    async uploadFile() {
      try {
        const formData = new FormData()
        formData.append('document', this.file)
        console.log(formData)
        await Axios.post('advertise-owner/document', formData)
        this.file = null
        ...
        this.$store.commit('setNotification', {
          title: 'Upload document not successfully',
          type: 'error',
          isShow: true,
        })
        console.log(error)
    ...
    // get advertiser documents from api
    async getDocument() {
      try {
        this.downloadedFile = await Axios.get('advertise-owner/document')
      } catch (error) {
        console.log(error)
      }
    },
