    ...
    <v-col cols="12">
      <v-form class="admin-sms-panel py-8">
        <v-row>
          ...
          >
            <v-text-field
              v-model="priceForEachSMS"
              label="Price For Each SMS"
              outlined
              placeholder="Price For Each SMS"
              hide-details
            ></v-text-field>
          </v-col>
          ...
          <v-col cols="12">
            <v-btn color="primary" @click="createSMSPanelLevel">
              Submit
            </v-btn>
<!--            <v-btn-->
<!--              type="reset"-->
...
<!--            </v-btn>-->
    ...
    <v-row justify="center">
      ...
      >
        <v-card>
          <v-card-title>
            <span class="text-h5">Update sms panel level</span>
          </v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Price For Each SMS', value: 'priceForEachSMS' },
  ...
  methods: {
    ...
    async createSMSPanelLevel() {
      try {
        await Axios.post('admin/level/sms-panel-level', {
          ...
          priceForEachSMS: this.priceForEachSMS,
          priceType: this.priceType,
        })
        this.$store.commit('setNotification', {
          title: 'Create sms panel level successfully',
          type: 'success',
          isShow: true,
        })
        await this.getAllSMSPanelLevel()
        ...
        this.priceForEachSMS = ''
        this.priceType = ''
      } catch (error) {
        this.$store.commit('setNotification', {
          title: 'Create sms panel level not successfully',
          type: 'error',
          isShow: true,
        })
      }
    },
    async updateSMSPanelLevel() {
      try {
        await Axios.put(`admin/level/sms-panel-level/${this.updateId}`, {
          marketerLevelId: this.marketerLevelId,
          marketerPercent: this.marketerPercent,
          price: this.price,
          parentMarketerPercent: this.parentMarketerPercent,
        ...
        this.$store.commit('setNotification', {
          title: 'Update sms panel level successfully',
          type: 'success',
          isShow: true,
        })
        this.dialog = false
        ...
        this.$store.commit('setNotification', {
          title: 'Update sms panel level not successfully',
          type: 'error',
          isShow: true,
        })
      } finally {
