  ...
  <v-row>
    <v-col cols="12">
      <v-card>
        <v-data-table
          :headers="headers"
          :items="buyLinkPanelList.data.data"
          item-key="full_name"
          class="table-rounded"
          hide-default-footer
          >
    ...
    <v-col cols="12">
      <v-form class="admin-sms-panel py-8">
        <v-row>
          ...
          >
            <v-text-field
              v-model="priceForEachBuy"
              label="Price For Each Buy"
              outlined
              placeholder="Price For Each Buy"
              hide-details
            ></v-text-field>
          </v-col>
          ...
          <v-col cols="12">
            <v-btn color="primary" @click="createBuyLinkPanelLevel">
              Submit
            </v-btn>
            <!--            <v-btn-->
            <!--              outlined-->
    ...
    <v-row justify="center">
      ...
      >
        <v-card>
          <v-card-title>
            <span class="text-h5">Update uy link panel level</span>
          </v-card-title>
          <v-card-text>
            <v-container>
              <v-row>
...
export default {
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'Price For Each Buy', value: 'priceForEachBuy' },
  ...
  methods: {
    ...
    async createBuyLinkPanelLevel() {
      try {
        await Axios.post('admin/level/buy-link-level', {
          ...
          priceForEachBuy: this.priceForEachBuy,
          priceType: this.priceType,
        })
        this.$store.commit('setNotification', {
          title: 'Create buy link panel level successfully',
          type: 'success',
          isShow: true,
        })
        await this.getAllBuyLinkPanelLevel()
        ...
        this.priceForEachBuy = ''
        this.priceType = ''
        this.priceForEachAct = ''
      } catch (error) {
        this.$store.commit('setNotification', {
          title: 'Create buy link panel level not successfully',
          type: 'error',
          isShow: true,
        })
      }
    },
    async updateBuyLinkPanelLevel() {
      try {
        await Axios.put(`admin/level/buy-link-level/${this.updateId}`, {
          marketerLevelId: this.marketerLevelId,
          marketerPercent: this.marketerPercent,
          price: this.price,
          parentMarketerPercent: this.parentMarketerPercent,
        ...
        this.$store.commit('setNotification', {
          title: 'Update buy link panel level successfully',
          type: 'success',
          isShow: true,
        })
        this.dialog = false
        ...
        this.$store.commit('setNotification', {
          title: 'Update buy link panel level not successfully',
          type: 'error',
          isShow: true,
        })
      } finally {
